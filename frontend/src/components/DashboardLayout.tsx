...
export default function DashboardLayout() {
  ...
  return (
    <div className="min-h-screen flex bg-gray-950">
      ...
      <aside className="hidden md:flex flex-col w-60 border-r border-white/5 py-5 px-4 gap-1 fixed top-0 left-0 h-full z-30">
        ...
        <div className="flex items-center gap-3 px-4 py-3 mb-4">
          <img
            src={user?.imageUrl}
            alt="avatar"
            className="w-8 h-8 rounded-full ring-2 ring-violet-500/40 flex-shrink-0"
          />
        ...
        <NavItem icon={LayoutDashboard} label="Home" to="/" />
        <NavItem icon={WandSparkles} label="Generate" to="/dashboard/generate" />
        <NavItem icon={FolderOpen} label="My Generations" to="/dashboard/my-generations" />
        <NavItem icon={DollarSign} label="Plans" to="/dashboard/plan" />
      ...
      {/* Mobile UserMenu (always visible on mobile) */}
      <div className="fixed top-4 left-4 z-50 md:hidden ">
        <UserMenu />
      </div>

      {/* ── Main Content — children render here ── */}
      <main className="flex-1 md:ml-60 px-4 md:px-8">
        <Outlet />
      </main>
